...
export const AdminPipeline: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 animate-fadeIn">
      {/* 管理ヘッダー */}
      <div className="bg-gradient-to-r from-amber-950 via-slate-900 to-slate-950 text-white p-6 sm:p-8 rounded-3xl shadow-xl border border-amber-500/30 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
        ...
        <button
          ...
          disabled={isCrawling}
          className="flex items-center space-x-2 px-5 py-3 rounded-2xl bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-600 hover:to-orange-700 text-white font-bold text-sm shadow-lg hover:shadow-xl transition-all active:scale-95 disabled:opacity-50"
        >
          {isCrawling ? (
            <>
      ...
      {/* 収集ログコンソール（デモ用） */}
      {crawlLogs.length > 0 && (
        <div className="bg-slate-950 p-5 rounded-2xl border border-slate-800 font-mono text-xs text-emerald-400 space-y-1 shadow-inner overflow-hidden">
          <div className="flex items-center justify-between text-slate-400 pb-2 border-b border-slate-800 mb-2">
            <div className="flex items-center space-x-2">
              <Terminal className="w-4 h-4 text-amber-400" />
              <span className="font-bold text-slate-200">crawler/collect_services.py 実行ログ</span>
            </div>
            <span className="text-[10px] text-slate-500">Structured Output Engine</span>
          </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
          <span className="text-xs text-slate-500 block">総サービス件数</span>
          <span className="text-2xl font-bold text-slate-900">{servicesList.length} 件</span>
        </div>
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="pl-9 pr-4 py-1.5 rounded-xl border border-slate-200 text-xs w-64 focus:outline-teal-600"
          />
        </div>
      </div>
                ...
                return (
                  <tr key={srv.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="p-3.5">
                      ...
                      <div className="text-[11px] text-slate-500">{srv.providerName}</div>
                    </td>
                    ...
                    <td className="p-3.5">
                      ...
                      <div className="text-[9px] text-slate-400">{srv.sourceType}</div>
                    </td>
                    ...
                    <td className="p-3.5 text-center whitespace-nowrap">
                      <div className="flex items-center justify-center space-x-1.5">
                        ...
                        <button
                          ...
                          className="px-2 py-1 rounded-lg bg-slate-200 hover:bg-rose-100 hover:text-rose-700 text-slate-700 text-[10px] font-medium transition-all"
                        >
                          却下
                        </button>
                      </div>
